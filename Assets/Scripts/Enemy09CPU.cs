...
public class Enemy09CPU : MonoBehaviour
{
    enum ActionStep
    {
        One, //移动到目标Y坐标
    ...
    bool DoFire = true;
    readonly float BulletAngle = 60f; //左右散弹初始角度
            ...
            if (CurrActionStep != ActionStep.Three)
            {
                ...
                if (Time.time > NextFireTime && CurrActionStep == ActionStep.Two)
                {
                    ...
                    if (DoFire)
                    {
                        ...
                        //两边的散弹
                        if (BulletAngleCurr == 0f)
                        {
                            BulletAngleCurr = BulletAngle;
                        }
                        Fire(bullet.eulerAngles.z + 90f - BulletAngleCurr);
                        Fire(bullet.eulerAngles.z + 90f + BulletAngleCurr);
                        BulletAngleCurr -= 3f;
                    }
                    else
