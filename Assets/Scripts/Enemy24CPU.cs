...
public class Enemy24CPU : MonoBehaviour
{
    enum Gun
    {
        Left = 0,
    ...
    public float FireTime03 = 0.8f; //子弹发射时间密度，越小越多子弹，这里AttackMode03会用到
    ...
    //左右散弹
    readonly float BulletAngleMin = 15f; //左右散弹最小角度
    readonly float BulletAngleMax = 40f; //左右散弹最大角度
    ...
    void Start()
    {
		...
		if (transform.parent.name != "HideLayer")
        {
            ...
            GunOffsetPosLeft = new Vector3(-0.69f, -1f, 0);
            GunOffsetPosMiddle = new Vector3(0, -0.7f, 0);
            GunOffsetPosRight = new Vector3(0.69f, -1f, 0);
    ...
    void AttackMode01()
    {
        if (Time.time > NextFireTime)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                Fire01(Gun.Left, 90 + angle + (i * 10));
                Fire01(Gun.Right, 90 - angle - (i * 10));
            }

            //中间
            for (int i = 0; i < 5; i++)
            {
                Fire02(Gun.Middle, Random.Range(180, 360), 5);
                Fire02(Gun.Middle, Random.Range(0, 83), 5);
                Fire02(Gun.Middle, Random.Range(97, 180), 5);
            }
    ...
    void AttackMode02()
    {
        if (Time.time > NextFireTime)
        {
            ...
            Fire01(Gun.Left, BulletAngleCurr);
            Fire01(Gun.Left, BulletAngleCurr + 20);
            Fire01(Gun.Left, BulletAngleCurr + 40);

            Fire01(Gun.Right, 180 - BulletAngleCurr);
            Fire01(Gun.Right, 160 - BulletAngleCurr);
            Fire01(Gun.Right, 140 - BulletAngleCurr);

            //中间
            Fire02(Gun.Middle, 80);
            Fire02(Gun.Middle, 90);
            Fire02(Gun.Middle, 100);
    ...
    void AttackMode03()
    {
        ...
        if (Time.time > NextFireTime03)
        {
            ...
            while (angle <= 360)
            {
                ...
                Fire01(Gun.Left, angle + 5, 3);
                Fire01(Gun.Left, angle - 5, 3);

                Fire01(Gun.Right, angle + Random.Range(-3f, 3f), Random.Range(3f, 4f));
